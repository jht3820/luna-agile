        ...
        var getBody = function getBody(_colGroup, _bodyRow, _groupRow, _list) {
            ...
            for (di = 0, dl = _list.length; di < dl; di++) {
                ...
                for (tri = 0, trl = rowTable.rows.length; tri < trl; tri++) {
                    ...
                    for (ci = 0, cl = rowTable.rows[tri].cols.length; ci < cl; ci++) {
                        col = rowTable.rows[tri].cols[ci];
                        SS.push('<td ', 'colspan="' + col.colspan + '" ', 'rowspan="' + col.rowspan + '" ', '>', (isGroupingRow ? getGroupingValue.call(this, _list[di], di, col) : getFieldValue.call(this, _list, _list[di], di, col, val, "text")) || '0', '</td>');
                    }
                    SS.push('\n</tr>');
                }
            }
