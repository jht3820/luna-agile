...
var boardNoticeGrid_config = {
      ...
      page: {
          ...
          onChange: function () {
             fnInGridListSetting(this.page.selectPage,"",gridObj["boardNotice"],"<c:url value='/bad/bad1000/bad1000/selectbad1000BoardListAjaxView.do'/>",false);
          }
      } 
  };
...
$(document).ready(function(){
	//가이드 상자 호출
	gfnGuideStack("add",fnDsh1000GuideShow);
...
function fnDashBoardSetting(){
	...
	$(".foldDiv[folding=notice]").show('fast',function(){
		$(".titleFoldingContent[folding=notice]").removeClass("down").addClass("up");	
	});
	...
	var ajaxObj = new gfnAjaxRequestAction(
		...
		//공지사항 그리드 세팅
		fnDashboardGridSetting("boardNotice");
		fnInGridListSetting(0,"",gridObj["boardNotice"],"<c:url value='/bad/bad1000/bad1000/selectbad1000BoardListAjaxView.do'/>",false);
		...
		fnInGridListSetting(0,"&signUsrId="+usrId,gridObj["sign"],"<c:url value='/chk/chk1000/chk1100/selectChk1100AjaxView.do'/>",false);
		
		//배포계획 결재 그리드 세팅
		fnDashboardGridSetting("dplSign");
		fnInGridListSetting(0,"&signUsrId="+usrId,gridObj["dplSign"],"<c:url value='/dpl/dpl2000/dpl2100/selectDpl2100AjaxView.do'/>",false);
		...
		$.each(processList,function(idx, map){
			...
			$("#process_list_box").append(
				'<div class="process_info_box '+setDisplayClass+'" processid="'+map.processId+'">'
...
function fnFlowListSetting(flowList){
	...
	$.each(flowList,function(idx, map){
		...
		$("div.process_kanban_box[processid="+map.processId+"]").append(
				'<div class="kanbanFrameDiv"  onclick="kanbanFlowFoldOff(this,\''+map.processId+'\')"onclick="fn" totalcnt="'+map.reqTotalCnt+'" style="background-color: '+map.flowTitleBgColor+';color: '+map.flowTitleColor+';">'
		...
		Sortable.create($(".kanbanContentReqList[processid="+map.processId+"][flowid="+map.flowId+"]")[0],{
			group: 'requestList',
...
function fnSubDataLoad(type,thisObj,thisProcessId){
	var redoId,processId;
	
	if(type == "obj"){
		var $targetObj = $(thisObj);
		redoId = $targetObj.attr("redoid");
		processId = $targetObj.attr("processid");
	}else{
	...
	//접수 요청
	if(redoId == "request"){
		fnInGridListSetting(gridObj["request"].page.currentPage,"&mode=dashboard&reqProType=01",gridObj["request"],"<c:url value='/req/req1000/req1000/selectReq1000ListAjaxView.do'/>",false);
		return false;
	}
	//공지사항
	else if(redoId == "boardNotice"){
	...
	ajaxObj.setFnSuccess(function(data){
		...
		if(!gfnIsNull(proFlowReqList)){
			...
			$.each(proFlowReqList,function(idx, map){
				...
				if(reqProType == "04"){
					//실패
					if(new Date(reqEdDuDtm) < reqEdDtm){
					...
					//임박
					else if(new Date(new Date(reqEdDuDtm).setDate(new Date(reqEdDuDtm).getDate()-3)) <= reqEdDtm){
...
function fnDsh1000GuideShow(){
	...
	//guide box setting
	var guideBoxInfo = globals_guideContents["dsh1000"];
...
<div class="main_contents">
	<div class="processFlowFullFrame"></div>
	<div class="contents_wrap">
		<div class="contents_title">
			${sessionScope.selMenuNm}
			<div class="dataAutoRefreshDiv" guide="autoRefresh">
		...
		<div class="osl_main_wrap">
			...
			<div class="dsh_top_box" guide="dshTopPrjBox">
				<div class="dsh_title_box">
					...
					<span id="dashboardPrjNm"><c:out value="${requestScope.prjNm}" /></span>
					<div class="dsh2000_dtmOverAlert" processid="project" guide="dtmOver">
			...
			<div class="dsh_middle_box" guide="request">
				<div class="dsh_title_box">
			...
			<div class="dsh_middle_box dsh_middle_half fr">
				<div class="dsh_title_box">
					<div class="titleFoldingBtn"><span class="titleFoldingContent up" folding="notice"></span></div>
					<div class="titleRedoBtn" redoid="boardNotice" onclick="fnSubDataLoad('obj',this);"><li class="fa fa-redo"></li></div>
					공지사항
				</div>
				<div class="foldDiv" folding="notice">
					<div data-ax5grid="dshGrid-boardNotice" data-ax5grid-config="{}" style="height: 250px;"></div>
				</div>
			</div>
			<div class="process_list_box" id="process_list_box" guide="processList">
				<!-- 프로세스 목록 세팅 영역 -->
			</div>
			<div class="dsh_middle_box dsh_middle_half fl">
				<div class="dsh_title_box">
					<div class="titleFoldingBtn"><span class="titleFoldingContent up" folding="dplSign"></span></div>
					<div class="titleRedoBtn" redoid="dplSign" onclick="fnSubDataLoad('obj',this);"><li class="fa fa-redo"></li></div>
					담당 배포계획 결재 목록
				</div>
				<div class="foldDiv" folding="dplSign">
