...
var OSLPrj3101Popup = function () {
    ...
    var documentSetting = function () {
		...
		$('#docSelectFormFileZipDown').click(function(){
			...
			//선택된 파일 수가 1개라면 1개만 다운로드
			if(checkedFileSn.length == 1){
				//파일 다운
				$.osl.file.fileDownload(atchFileId, checkedFileSn[0]);
			}
			//선택된 파일 수가 2개 이상이라면 압축 다운로드
			else if(checkedFileSn.length > 1){
