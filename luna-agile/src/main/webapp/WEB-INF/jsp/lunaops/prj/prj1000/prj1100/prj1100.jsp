...
<link rel='stylesheet' href='<c:url value='/css/common/spectrum.css'/>'
	type='text/css'>
<link rel='stylesheet'
	href='<c:url value='/css/flowchart/jquery.flowchart.css'/>'
	type='text/css'>
<link rel='stylesheet' href='<c:url value='/css/oslops/flw.css'/>'
	type='text/css'>
<script>
	//ie 함수 재정의
	...
	$(function() {
		...
		$("#previewFlowTitleBgColor").spectrum({
			showInput : true,
			chooseText : "선택",
			cancelText : "닫기",
		...
		$("#previewFlowTitleColor").spectrum({
			showInput : true,
			chooseText : "선택",
			cancelText : "닫기",
			preferredFormat : "name",
		...
		$("#previewFlowContentBgColor").spectrum({
			showInput : true,
			chooseText : "선택",
			cancelText : "닫기",
			preferredFormat : "name",
		...
		$("#previewFlowContentColor").spectrum({
			showInput : true,
			chooseText : "선택",
			cancelText : "닫기",
			preferredFormat : "name",
		...
		$("#btn_insert_proInsert").click(
				function() {
					...
					gfnLayerPopupOpen(
							"/prj/prj1000/prj1100/selectPrj1105View.do", data,
							'550', '470', 'scroll');
				});

		//프로세스 수정 버튼
		$("#btn_update_proUpdate").click(
				function() {
		...
		//프로세스 삭제 버튼
		$("#btn_delete_proDelete")
				.click(
						function() {
							//확정 프로세스 삭제 불가
							var processConfirmCd = $(".process_box.active")
									.attr("confirm");
							...
							jConfirm(
									...
									function(result) {
										if (result) {
											...
											//AJAX 전송 오류 함수
											ajaxObj.setFnError(function(xhr,
													status, err) {
												data = JSON.parse(data);
				...
				.click(
						function() {
							...
							if (processConfirmCd == "02") {
								...
								jConfirm(
										"프로세스를 확정하시겠습니까?<br>확정된 프로세스의 작업흐름은 기본 정보만 수정이 가능합니다.<br>(작업흐름명, 색상, 위치)",
										"알림", function(result) {
											if (result) {
												fnProcessConfirm();
											}
				...
				.click(
						function() {
							...
							if (processConfirmCd == "01") {
								...
								jConfirm(
										"프로세스를 확정 취소하시겠습니까?<br>1개 이상의 요구사항이 배정되어있으면 취소가 불가능합니다.",
										"알림", function(result) {
											if (result) {
		...
		//프로세스 복사 버튼
		$("#btn_delete_proCopy").click(
				function() {
					//팝업창 오픈
					var data = {};
					gfnLayerPopupOpen(
							"/prj/prj1000/prj1100/selectPrj1104View.do", data,
							'1330', '970', 'auto');
				});

		/************* 작업흐름 제어 시작 **/
		//작업흐름 추가
		$("#btn_insert_flwInsert").click(
				function() {
					//프로세스 Id
					var processId = $(".process_box.active").attr('id');

					//확정 프로세스 추가 불가
					var processConfirmCd = $(".process_box.active").attr(
							"confirm");
					if (processConfirmCd == "02") {
					...
					gfnLayerPopupOpen(
							"/prj/prj1000/prj1100/selectPrj1102View.do", data,
							'1100', '610', 'scroll');
				});

		//작업흐름 수정
		$("#btn_update_flwUpdate").click(
				function() {
		...
		$("#btn_delete_flwDelete").click(
				function() {
					...
					jConfirm("작업흐름을 삭제하시겠습니까?<br>연결된 링크가 함께 삭제됩니다.", "알림",
							function(result) {
								if (result) {
									fnFlowDeleteAjax();
								}
		...
		//선택 작업흐름 추가항목 관리
		$("#btn_insert_flwItemInsert").click(
				function() {
					//확정 처리된 프로세스인지 확인
					var processConfirmCd = $(".process_box.active").attr(
							"confirm");
					...
					gfnLayerPopupOpen(
							"/prj/prj1000/prj1100/selectPrj1101View.do", data,
							'1100', '600', 'scroll');
				});
	...
	function fnProcessSaveAjax() {
		...
		if (!gfnIsNull(processNm)) {
			...
			var processDesc = encodeURI(gfnCutStrLen($("#processDesc").val(),
					4000));
			...
			var ajaxObj = new gfnAjaxRequestAction({
				...
				processConfirmCd : processConfirmCd
			});
			//AJAX 전송 성공 함수
	...
	function fnProcessConfirm() {
		...
		$.each(flowKeys, function(idx, map) {
			...
			if (gfnIsNull(flowData.properties.flowNextId)
					|| flowData.properties.flowNextId == "null") {
				lastFlowIdx = idx;
				linkChk++;
				return true;
			}
	...
	function fnProcessConfirmCancel() {
		...
		var ajaxObj = new gfnAjaxRequestAction(
				{
					"url" : "<c:url value='/prj/prj1000/prj1100/selectPrj1100ProcessReqCntAjax.do'/>",
					async : true
				}, {
					processId : processId
				});
		...
		ajaxObj
				.setFnSuccess(function(data) {
					...
					if (data.errorYN == "Y") {
						...
						if (proReqCnt > 0) {
							jAlert("프로세스에 배정된 요구사항이 존재합니다.<br>확정 취소 할 수 없습니다.",
									"알림");
						} else {
							//현재 프로세스 데이터
							var flowGetData = $flowchart.flowchart('getData');
							...
							//작업흐름 목록 루프
							if (!gfnIsNull(flowGetData)
									&& !gfnIsNull(flowGetData.operators)) {
								//최종완료 찾기
								$
										.each(
												flowGetData.operators,
												function(idx, map) {
													//properties
							...
							var ajaxObj = new gfnAjaxRequestAction(
									{
										"url" : "<c:url value='/prj/prj1000/prj1100/updatePrj1100ProcessConfirmCancelAjax.do'/>",
										async : true
									}, {
										processJsonData : JSON
												.stringify(tmp_flowData),
										processId : processId,
										endFlowId : endFlowId,
							...
							ajaxObj.setFnSuccess(function(data) {
								...
								if (data.errorYN == "Y") {
									...
									//프로세스 확정 취소
									$(".process_box.active").attr("confirm",
											"01");
									...
									//데이터 갱신
									processJsonList[processId] = JSON
											.stringify(tmp_flowData);
								}
							});
	...
	function fnFlowLinkConnAjax(linkId, linkData, type) {
		...
		//해당 작업흐름 정보 불러오기
		var preFlowInfo = $flowchart.flowchart("getOperatorData",
		...
		//작업흐름 데이터 교체
		$flowchart.flowchart("setOperatorData", linkData.fromOperator,
				preFlowInfo);
	...
	function fnProcessListAjax() {
		...
		var ajaxObj = new gfnAjaxRequestAction(
				{
					"url" : "<c:url value='/prj/prj1000/prj1100/selectPrj1100ProcessListAjax.do'/>",
					"loadingShow" : false
				});
		//AJAX 전송 성공 함수
		ajaxObj
												...
												$("#processBoxList")
														.append(
																'<div class="process_box '
																		...
																		+ map.processId
																		+ '" confirm="'
																		+ map.processConfirmCd
																		+ '" desc="'
																		+ map.processDesc
																		+ '" ord="'
																		+ map.processOrd
																		+ '" title="'
																		+ decodeURI(map.processDesc)
																		+ '">&nbsp;'
																		+ map.processNm
																		+ '<div class="prj1100ProcessDtm">(생성일: '
																		+ map.regDtm
																		+ ')</div></div>');
															...
															.flowchart({
																			...
																			.spectrum(
																					{
																						...
																						cancelText : "닫기",
																						preferredFormat : "name",
																						color : selFlowPro.flowTitleBgColor
																					});
																	$(
																			"#previewFlowTitleColor")
																			.spectrum(
																					{
																						showInput : true,
																						chooseText : "선택",
																						cancelText : "닫기",
																						preferredFormat : "name",
																						color : selFlowPro.flowTitleColor
																					});
																	$(
																			"#previewFlowContentBgColor")
																			.spectrum(
																					{
																						showInput : true,
																						chooseText : "선택",
																						cancelText : "닫기",
																						preferredFormat : "name",
																						color : selFlowPro.flowContentBgColor
																					});
																	$(
																			"#previewFlowContentColor")
																			.spectrum(
																					{
																						showInput : true,
																						chooseText : "선택",
																						cancelText : "닫기",
																						preferredFormat : "name",
																						color : selFlowPro.flowContentColor
																					});
																...
																onLinkCreate : function(
																	...
																	if (linkData.linkCreateEnd != "Y") {
																		...
																		var flowGetData = $flowchart
																				.flowchart('getData');

																		//실제로 링크 데이터가 남아 있는 경우 체크
																		if (flowGetData.links
																...
																//링크 삭제
																onLinkDelete : function(
																		linkId,
																		forced) {
																	//링크 제거된 경우 이전에 연결되어있던 작업흐름 nextId null
																	var linkData = {
																		fromOperator : $flowchart
																				.flowchart('getData').links[linkId].fromOperator
																	};
																	fnFlowLinkConnAjax(
																			linkId,
																			linkData,
									...
									.click(
											function() {
												...
												if (!gfnIsNull(processJsonList[processId])) {
													jsonData = JSON
															.parse(processJsonList[processId]);
												}
	...
	function fnAxGrid5View() {
		...
		firstGrid.setConfig({
			...
			columns : [ {
				...
				align : "center",
				formatter : function() {
					return (this.item.itemEssentialCd == "01") ? "Y" : "N";
				}
			}, {
				key : "itemModifyCd",
				label : "수정",
				...
				align : "center",
				formatter : function() {
					return (this.item.itemModifyCd == "01") ? "Y" : "N";
				}
			}, {
				key : "itemNm",
				label : "항목 명",
	...
	function fnColorFixReset() {
		...
		if (!gfnIsNull(selFlowId)) {
			//작업흐름 원본 데이터
			var selOperatorData = $flowchart.flowchart("getOperatorData",
					selFlowId).properties;

			//원본 데이터 있는 경우에만
			if (!gfnIsNull(selOperatorData)) {
				//작업흐름 색상 초기화
				fnColorFixSet("operator", {
					"background-color" : selOperatorData.flowContentBgColor,
					"color" : selOperatorData.flowContentColor
				});
				fnColorFixSet("title", {
					"background-color" : selOperatorData.flowTitleBgColor,
					"color" : selOperatorData.flowTitleColor
				});
			}
		}
	}
	...
	function fnPreviewAuthRefresh(flowId, authGrpTargetCd, gridTarget) {
		...
		var ajaxObj = new gfnAjaxRequestAction(
				{
					"url" : "<c:url value='/prj/prj1000/prj1100/selectPrj1100FlowAuthGrpListAjax.do'/>",
					"loadingShow" : false
				}, {
					processId : processId,
					flowId : flowId,
					authGrpTargetCd : authGrpTargetCd
				});
		//AJAX 전송 성공 함수
		ajaxObj.setFnSuccess(function(data) {
			data = JSON.parse(data);
	...
	function fnFlowMoveSave() {
		...
		//AJAX 설정
		var ajaxObj = new gfnAjaxRequestAction({
			"url" : "/prj/prj1000/prj1100/updatePrj1100ProcessInfoAjax.do",
			"async" : true
		}, {
			processJsonData : JSON.stringify(flowGetData),
			processId : processId
...
<div class="main_contents">
	...
	<div class="flw_main_middleBox_save" id="flw_main_middleBox_save">
		<div class="flw_title_box">
			프로세스 <span id="processActionTypeNm">등록</span>
		</div>
		<div class="process_regFrame">
			<div class="processNm">
				<div class="process_regTitle">
					<label for="processNm">프로세스 명</label>
				</div>
				<input type="text" name="processNm" id="processNm"
					placeholder="프로세스명을 입력해주세요." />
			</div>
			<div class="processDesc">
				<div class="process_regTitle">
					<label for="processDesc">프로세스 설명</label>
				</div>
				<textarea name="processDesc" id="processDesc"></textarea>
			</div>
			<div class="processOrd">
				<div class="process_regTitle">
					<label for="processOrd">프로세스 순서</label>
				</div>
				<input type="number" name="processOrd" id="processOrd"
					placeholder="프로세스 순서를 입력해주세요." value="0" min="0" />
			</div>
			<div class="processBtn">
				<span class="button_normal2" id="btn_insert_plcInsert">프로세스
					저장</span> <span class="button_normal2" id="btn_insert_plcCancle">저장
					취소</span>
			</div>
		</div>
	</div>
	<div class="flw_mainFrame" id="flw_mainFrame">
		<div class="zoomBtn" guide="zoom">
			<span class="button_normal2" onclick="fnFlowChartZoom('reset')"><i
				class="fa fa-undo-alt"></i></span> <span class="button_normal2"
				onclick="fnFlowChartZoom('in')"><i class="fa fa-plus"></i></span> <span
				class="button_normal2" onclick="fnFlowChartZoom('out')"><i
		...
		<div class="flw_mainBox" id="flw_mainBox">
			<div class="flw_box flw_main_leftBox">
				<div class="flw_box flw_left_topBox" guide="leftMenu">
					<span class="button_normal2" id="btn_insert_proSelect">조회</span> <span
						class="button_normal2" id="btn_insert_proInsert">추가</span> <span
						class="button_normal2" id="btn_update_proUpdate">수정</span> <span
						class="button_normal2" id="btn_delete_proDelete">삭제</span> <span
						class="button_normal2" id="btn_delete_proCopy">복사</span> <span
						class="button_normal2" id="btn_update_proConfirm">확정</span> <span
						class="button_normal2" id="btn_update_proConfirmCancel">확정취소</span>
				</div>
				<div class="flw_box_clear"></div>
				<div class="flw_box flw_left_bottomBox" id="processBoxList"></div>
			</div>
			...
			<div class="flw_box flw_main_middleBox" guide="functionList">
				...
				<span class="button_normal2" id="btn_insert_flwInsert"><i
					class="fa fa-plus"></i>&nbsp;작업흐름 추가</span> <span class="button_normal2"
					id="btn_update_flwUpdate"><i class="fa fa-edit"></i>&nbsp;선택
					작업흐름 수정</span> <span class="button_normal2" id="btn_delete_flwDelete"><i
					class="fa fa-times"></i>&nbsp;선택 작업흐름 삭제</span> <span
					class="button_normal2" id="btn_delete_flwLinkDelete"><i
					class="fa fa-times"></i>&nbsp;선택 작업흐름 링크 삭제</span>
				<!-- <span class="button_normal2" id="btn_update_flwReset"><i class="fa fa-history"></i>&nbsp;내용 초기화</span> -->
				<span class="button_normal2" id="btn_insert_flwItemInsert"><i
					class="fa fa-list"></i>&nbsp;추가 항목 관리</span>
			</div>
			<div class="flw_box_clear"></div>
			<div class="flw_box flw_main_bottomBox" guide="flowInfo">
				<form name="flowInfo" id="flowInfo">
					<div class="flw_content_mask" id="flowContentMask">작업흐름을
						선택해주세요.</div>
					<div class="flw_sub_box flw_bottom_leftBox" style="height: auto;">
						...
						<div class="flw_sub_box flw_sub3">
							<input type="text" name="previewFlowNm" id="previewFlowNm"
								readonly="readonly" style="border: 1px solid #ccc;" />
						</div>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right"
							title="선택 작업흐름을 필수 단계로 지정합니다.">필수</div>
						<div class="flw_sub_box flw_sub1 flw_line_right">
							<div class="flw_chk">
								<input type="checkbox" title="체크박스" name="previewFlowEssential"
									id="previewFlowEssential" disabled="disabled" /><label></label>
							</div>
						</div>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right">
							제목 배경 색상</div>
						<div class="flw_sub_box flw_sub1">
							<input type="color" name="previewFlowTitleBgColor"
								id="previewFlowTitleBgColor" value="#515769" disabled="disabled" />
						</div>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right"
							title="다음 작업흐름 변경시 결재를 받도록 지정합니다.">결재 요청</div>
						<div class="flw_sub_box flw_sub1 flw_line_right">
							<div class="flw_chk">
								<input type="checkbox" title="체크박스" name="previewFlowSign"
									id="previewFlowSign" disabled="disabled" /><label></label>
							</div>
						</div>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right">
							제목 글씨 색상</div>
						<div class="flw_sub_box flw_sub1">
							<input type="color" name="previewFlowTitleColor"
								id="previewFlowTitleColor" value="#FFFFFF" disabled="disabled" />
						</div>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right"
							title="다음 작업흐름 변경시 바로 최종완료 작업흐름으로 변경이 가능하도록 합니다.">종료 분기</div>
						<div class="flw_sub_box flw_sub1 flw_line_right">
							<div class="flw_chk">
								<input type="checkbox" title="체크박스" name="previewFlowEnd"
									id="previewFlowEnd" disabled="disabled" /><label></label>
							</div>
						</div>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right">
							내용 배경 색상</div>
						<div class="flw_sub_box flw_sub1">
							<input type="color" name="previewFlowContentBgColor"
								id="previewFlowContentBgColor" value="#FFFFFF"
								disabled="disabled" />
						</div>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right"
							title="현재 작업흐름에서 리비전 번호를 입력 받도록 지정합니다.">허용 역할</div>
						<div class="flw_sub_box flw_sub1 flw_line_right">
							<div class="flw_chk">
								<input type="checkbox" title="체크박스" name="previewFlowAuth"
									id="previewFlowAuth" disabled="disabled" /><label></label>
							</div>
						</div>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right">
							내용 글씨 색상</div>
						<div class="flw_sub_box flw_sub1">
							<input type="color" name="previewFlowContentColor"
								id="previewFlowContentColor" value="#515769" disabled="disabled" />
						</div>
						<c:choose>
							<c:when
								test="${empty jenkinsModuleUseChk or jenkinsModuleUseChk == true }">
								<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right">
									배포 계획 저장</div>
								<div class="flw_sub_box flw_sub1 flw_line_right">
									<div class="flw_chk">
										<input type="checkbox" title="체크박스" name="previewFlowDpl"
											id="previewFlowDpl" disabled="disabled" /><label></label>
									</div>
								</div>
							</c:when>
							<c:otherwise>
								...
								<div class="flw_sub_box flw_sub1 flw_line_right">
									<div class="flw_chk">
										<input type="hidden" title="체크박스" name="previewFlowDpl"
											id="previewFlowDpl" class="optPreviewCdChg" opt="optDpl" />
									</div>
								</div>
							</c:otherwise>
						</c:choose>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right"
							title="현재 작업흐름에서 작업을 추가 할 수 있도록 지정합니다.">작업</div>
						<div class="flw_sub_box flw_sub1">
							<div class="flw_chk">
								<input type="checkbox" title="체크박스" name="previewFlowWork"
									id="previewFlowWork" disabled="disabled" /><label></label>
							</div>
						</div>
						<c:choose>
							<c:when
								test="${empty svnkitModuleUseChk or svnkitModuleUseChk == true }">
								<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right"
									title="현재 작업흐름에서 리비전 번호를 입력 받도록 지정합니다.">리비전 저장</div>
								<div class="flw_sub_box flw_sub1 flw_line_right">
									<div class="flw_chk">
										<input type="checkbox" title="체크박스" name="previewFlowRevision"
											id="previewFlowRevision" disabled="disabled" /><label></label>
									</div>
								</div>
							</c:when>
							<c:otherwise>
								<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right"
									title="현재 작업흐름에서 리비전 번호를 입력 받도록 지정합니다."></div>
								<div class="flw_sub_box flw_sub1 flw_line_right">
									<input type="hidden" title="체크박스" name="previewFlowRevision"
										id="previewFlowRevision" disabled="disabled" />
								</div>
							</c:otherwise>
						</c:choose>
						<div class="flw_sub_box flw_sub_title flw_sub1 flw_line_right">
						</div>
						<div class="flw_sub_box flw_sub1"></div>
						<div
							class="flw_sub_box flw_sub_title flw_sub1 flw_sub_desc flw_auth_frame">
							담당자</br>허용 역할 목록
						</div>
						<div
							class="flw_sub_box flw_sub3 flw_sub_desc flw_line_bottom_none flw_auth_frame">
							<div data-ax5grid="previewAuth-grid" data-ax5grid-config="{}"
					...
					<div class="flw_sub_box flw_bottom_rightBox" style="height: auto;">
						...
						<div class="flw_sub_box flw_sub4" style="height: 240px;">
							<div data-ax5grid="flw-grid" data-ax5grid-config="{}"
								style="height: 240px;"></div>
						</div>
						<div
							class="flw_sub_box flw_sub_title flw_sub1 flw_sub_desc flw_line_left flw_auth_frame">
							결재자</br>허용 역할 목록
						</div>
						<div
							class="flw_sub_box flw_sub3 flw_sub_desc flw_line_bottom_none flw_auth_frame">
							<div data-ax5grid="previewSignAuth-grid" data-ax5grid-config="{}"
								style="height: 100px;"></div>
						</div>
					</div>
				</form>
