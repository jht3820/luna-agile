	...
	firstGrid = new ax5.ui.grid();
        ...
        firstGrid.setConfig({
            ...
            body: {
                ...
                columnHeight: 30,
                onDBLClick:function(){
                	var data = {"popupGb": "update", "apiId": firstGrid.list[this.doindex].apiId};
                	                	
					gfnLayerPopupOpen('/stm/stm1000/stm1000/selectStm1001View.do',data,"680","730",'scroll');
                }
            },
            page: {
...
function fnInGridListSet(_pageNo,ajaxParam){
		...
		var ajaxObj = new gfnAjaxRequestAction(
				{"url":"<c:url value='/stm/stm1000/stm1000/selectStm1000ListAjaxView.do'/>","loadingShow":true}
				,ajaxParam);
		//AJAX 전송 성공 함수
...
function fnDeleteApi1000Info(apiId){
	//AJAX 설정
	var ajaxObj = new gfnAjaxRequestAction(
			{"url":"<c:url value='/stm/stm1000/stm1000/deleteStm1000InfoAjax.do'/>"}
			,{ "apiId" : apiId });
	//AJAX 전송 성공 함수
	ajaxObj.setFnSuccess(function(data){
		data = JSON.parse(data);
					...
					{display:true, addClass:"", style:"", list:[
						...
						{label : "",labelWidth : "",type : "button",width : "55",key : "btn_delete_req",style : "float:right;",valueBoxStyle : "padding:5px;",value : "<i class='fa fa-trash-alt' aria-hidden='true'></i>&nbsp;<span>삭제</span>",
							onclick : function() {
								...
								if(confirm("삭제 하시겠습니까?")){
									fnDeleteApi1000Info(item.apiId);
								}
							}
						},
						{label:"", labelWidth:"", type:"button", width:"60",style:"float:right;", key:"btn_update_api",valueBoxStyle:"padding:5px;", value:"<i class='fa fa-edit' aria-hidden='true'></i>&nbsp;<span>수정</span>",
							onclick:function(){
								...
								var data = {"popupGb": "update", "apiId": item.apiId};
