...
public class Adm1000Controller {
	...
	public String selectAdm1000View(HttpServletRequest request, HttpServletResponse response, ModelMap model)
			throws Exception {
	...
	public String selectAdm1001View(HttpServletRequest request, HttpServletResponse response, ModelMap model)
			throws Exception {
	...
	public ModelAndView selectAdm1000MenuInfoAjax(HttpServletRequest request, HttpServletResponse response,
	...
	public ModelAndView selectAdm1000MenuListAjax(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {
	...
	public ModelAndView selectAdm1000AuthGrpMenuListAjax(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {
	...
	public ModelAndView saveAdm1000AuthGrpMenuAuthListAjax(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {
		try {
			/**
			 * hidden + menuId + 컬럼명 으로 넘어오는 인풋 정보를 메뉴ID 별로 컬럼값 세팅하여 리스트로 담은 후 저장 처리함.
			 */
			...
			while (enu.hasMoreElements()) {
				...
				if ("hidden".equals(strKeys.substring(0, 6))) {
					 ...
					 * if(tempMenuId.equals(menuId)){ chkMap.put(colNm,
					 * request.getParameter("hidden" + menuId + colNm));
					 * 
					 * //컬럼명이 printYn 이면 마지막 컬럼 이므로 맵을 리스트에 저장한다. if(colNm.equals("printYn")){
					 * chkMap.put("authGrpId", authGrpId); list.add(chkMap);
					 * System.out.println(list); } } //이전 저장된 menuId와 다르면 새로운 메뉴정보 이므로 새로이 맵을 만들어서
					 * menuId를 맵에 넣는다. else{ chkMap = new HashMap();
					 * 
					 * //기본정보 생성된 맵에 붙이기 RequestConvertor.mapAddCommonInfo(request, chkMap);
					 * 
					 * //관리자이기에 prjId 는 ROOTSYSTEM_PRJ로 세팅한다. chkMap.put("prjId", "ROOTSYSTEM_PRJ");
					 * 
					 * if(menuId.equals(strStatusMenuId)){ chkMap.put("status", strStatus); }
					 * 
					 * //메뉴정보 세팅 chkMap.put("menuId", menuId); chkMap.put(colNm,
					 * request.getParameter("hidden" + menuId + colNm)); } tempMenuId = menuId;
					 */
				}
			}
	...
	/**
	 * Adm1000 메뉴정보 등록(단건) AJAX 메뉴정보 등록후 등록 정보 결과 및 정보를 화면으로 리턴한다.
	 * 
	 * @param
	 * @return
	 * @exception Exception
	...
	public ModelAndView insertAdm1000MenuInfoAjax(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {
	...
	/**
	 * Adm1000 메뉴정보 삭제(단건) AJAX 메뉴정보 삭제 처리
	 * 
	 * @param
	 * @return
	 * @exception Exception
	...
	public ModelAndView deleteAdm1000MenuInfoAjax(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {
	...
	/**
	 * Adm1000 메뉴정보 수정(단건) AJAX 메뉴정보 수정 처리
	 * 
	 * @param
	 * @return
	 * @exception Exception
	...
	public ModelAndView updateAdm1000MenuInfoAjax(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {
	...
	/**
	 * Adm1000 신규권한 등록 (단건) AJAX 신규권한 등록 처리
	 * 
	 * @param
	 * @return
	 * @exception Exception
	...
	public ModelAndView insertAdm1000AuthGrpInfoAjax(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {
	...
	/**
	 * Adm1000 권한정보 삭제(단건) AJAX 메뉴정보 삭제 처리
	 * 
	 * @param
	 * @return
	 * @exception Exception
	...
	public ModelAndView deleteAdm1000AuthGrpInfoAjax(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {
	...
	public ModelAndView selectAdm1000PrjAuthGrpList(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {
